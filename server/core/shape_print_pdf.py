...
def generate_shape_print_pdf(output_path: str, raster_only_pdf_path: str, grid: Grid) -> None:
    ...
    # and only get cleaned up in the finally around that whole span.
    ...
    try:
        ...
        # Layer 2 — artwork tiled into every cell (top). No contain-fit here —
        # the tile IS the cell (bleed-inclusive), so scale is always 1:1; only
        # the placement translation varies per cell.
        ...
        # Close the reader's own file handle explicitly before removing the
        # temp file — Windows raises PermissionError deleting a file that
        # still has a live handle, and merge_page's laziness means the
        # reader must stay open (not closed early) all the way through
        # writer.write() above, so this can't happen any sooner than here.
